...
const updateProfile = async (req, res) => {
  try {
      ...
      if (newPassword) {
          ...
          if (!isMatch) {
              return res.redirect(`/profile?error=invalidPassword`);
          }
          // Check if new password and confirm new password match
          if (newPassword !== confirmNewPassword) {
              return res.redirect(`/profile?error=passwordMismatch`);
          }
      ...
      await user.save();
      // Pass success message as a query parameter
      res.redirect('/profile?success=passwordChanged');
  } catch (error) {
      res.status(500).send(error.message);
  }
};
...
const addToCart = async (req, res) => {
  try {
      ...
      await cart.save();
      existingItem = cart.items.find(item => item.productId.toString() === productId);

      res.json({ success: true, cartItemCount: existingItem.quantity });
  } catch (error) {
      console.error('Error adding to cart:', error);
      res.status(500).json({ success: false, message: 'Error adding to cart' });
  }
...
const checkoutDeleteAddress = async (req, res) => {
  try {
    ...
    // Fetch all updated addresses after deleting
    const addresses = await Address.find({ user: userId });
